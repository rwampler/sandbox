...
import EventListener from '~/plugins/hoa/client/event-listener.coffee'
...
export default class Mouse
  ...
  initialize_with_dom: (dom=null) ->
    @parent_dom = if dom? then dom else document
  ...
  start_moving: (event) ->
    return unless @state_manager.state.initialized && @state_manager.state.status == 'ready'
    @is_moving = true
  ...
  finish_moving: (event) =>
    return unless @state_manager.state.initialized && @state_manager.state.status == 'ready' && @is_moving
    @is_moving = false
